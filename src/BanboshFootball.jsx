...
const TX={
  en:{title:"BANBOSH <b>FOOTBALL</b>",sub:"Minimalist football duel — play AI, tournament or online.",
    ...
    termsText:"By using Banbosh Football, you agree to use the game for entertainment purposes only. The game is provided as-is without warranties.",created:"Created by",weather:"Weather",weatherClear:"Clear",weatherRain:"Rain",weatherSnow:"Snow",weatherRandom:"Random"},
...
class Music{
  ...
  _sync(){
    this.init();
    const ensurePlaying=a=>{if(a&&a.paused){try{a.play().catch(()=>{})}catch(e){}}};
    const ensurePaused=a=>{if(a&&!a.paused){try{a.pause()}catch(e){}}};
    if(this.muted||(!this._wantMenu&&!this._wantGame)){ensurePaused(this.menu);ensurePaused(this.amb1);ensurePaused(this.amb2);return}
    if(this._wantMenu){ensurePaused(this.amb1);ensurePaused(this.amb2);ensurePlaying(this.menu)}
    else if(this._wantGame){ensurePaused(this.menu);ensurePlaying(this.amb1);ensurePlaying(this.amb2)}
  }
  ...
  unlock(){
    ...
    // Sync intentional state inside the gesture so play() is allowed
  ...
  if(scr==="jersey1"){const ds=[{key:"easy",em:"⭐"},{key:"medium",em:"⭐⭐"},{key:"hard",em:"⭐⭐⭐"}];
    ...
    <div style={{...panel,padding:"18px 16px"}}>
      ...
      {sel&&mode==="ai"&&<>
        ...
        <div style={{fontWeight:700,fontSize:"0.95em",marginTop:10,marginBottom:4}}>{t("weather")}</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center"}}>
          {[{k:"clear",em:"☀️",lk:"weatherClear"},{k:"rain",em:"🌧️",lk:"weatherRain"},{k:"snow",em:"❄️",lk:"weatherSnow"},{k:"random",em:"🎲",lk:"weatherRandom"}].map(w=>(
            <button key={w.k} onClick={()=>{sfx.click();setAiWeather(w.k)}} style={optBtn(aiWeather===w.k,{minWidth:70})}>{w.em} {t(w.lk)}</button>
          ))}
        </div>
      </>}
    </div>
